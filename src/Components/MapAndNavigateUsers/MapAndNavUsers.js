...
const Helper = ({ users, isLoading }) => {
  ...
  const UserDetailsPage=(user)=>{
    navigate('/:' + user.login.username ,{state:{location: user.location ,img: user.picture.large, fname: user.name.first[0] + ". " + user.name.last, email: user.email, gender:user.gender, age: user.dob.age}});
      }
